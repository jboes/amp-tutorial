...
In the Kitchin group, we use the Vienna /ab initio/ simulation package (VASP) suit to perform Density functional theory (DFT).
...
for atoms in traj:
   ...
   atoms.set_calculator(Amp('networks/3-3/'))
   E3 += [atoms.get_potential_energy()]

# We can use this to visualize every 10th step of the MD simulation.
#view(traj)
