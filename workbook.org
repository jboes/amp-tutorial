...
[calc.write2db('database/DFT.db', parser='=') for calc in calcs]
#+END_SRC
...
nrg, t, kpts = [], [], []
with connect('database/DFT.db') as db:
...
# Write all entries to database
[calc.write2db('database/DFT.db', parser='=')
 for calc in calcs]
#+END_SRC
...
calc.write2db('database/DFT.db', parser='=', keys={'dbkey': 0})
#+END_SRC
...
db = connect('database/DFT.db')

n_ids = []
for d in db.select('conv=None'):
    n_ids += [d.id]
...
for n in [2, 3]:
    ...
    calc.train(images=images,
               cores=1,
               global_search=None, # This is not useful for such a small NN
               extend_variables=False) # Do not use this feature
#+END_SRC
