...
if ready:
    ...
    ax2.set_ylabel('Absolute residual error (eV/atom)', color='r')
...
Training the second networks took significantly longer since we are no longer training such simple structures. There is now a need for more than one descriptor to define the system. Fortunately the 
