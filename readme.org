...
- More information can be found in Reference cite:Behler_2007.
...
[calc.write_db('database/master.db',
...
calc.write_db('database/master.db', parser='=',
...
calc2 = Amp('./networks/db1/2-2/checkpoint-parameters.json')
calc3 = Amp('./networks/db1/3-3/checkpoint-parameters.json')
...
for i, n in enumerate([2, 3]):
    ...
    E, nnE, var, ind = [], [], [], []
    for j, d in enumerate(db.select('dbkey')):
	E += [d.energy / d.natoms]
...
onnE, nnE = [], []
for i, atoms in enumerate(Trajectory('MD/db0/bulk.traj', 'r')):
    onnE += [atoms.get_potential_energy()]
    atoms.set_calculator(Amp('networks/db1/3-3/checkpoint-parameters.json'))
    nnE += [atoms.get_potential_energy()]

E = []
for d in db.select('MD'):
    res = abs(nnE[d.MD] - d.energy)
    E += [[d.MD, d.energy, res]]
...
fig, ax = plt.subplots(figsize=(6, 4))
ax.plot(range(len(onnE)), onnE, 'b--', label='db0 NN prediction')
ax.plot(range(len(nnE)), nnE, 'b-', label='db1 NN prediction')
ax.scatter(E[0], E[1], facecolor='none', edgecolor='b', label='DFT prediction')

ax.set_xlim(min(E[0]), max(E[0]))
ax.set_ylabel('Potential energy (eV)', color='b')
ax.tick_params(axis='y', colors='b')
ax.set_ylim(-141, -130)
ax.set_xlabel('Time step')
...
ax1.scatter(E[0], E[2], color='r')
ax1.set_ylabel('Absolute residual error (eV)', color='r')
ax1.set_xlim(min(E[0]), max(E[0]))
ax1.tick_params(axis='y', colors='r')
ax1.set_ylim(0, 7)
